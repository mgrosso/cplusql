...
AC_DEFUN([AC_CXX_HAVE_LOGL],
[AC_CACHE_CHECK(whether long double logl is supported,
ac_cv_cxx_have_logl,
[AC_LANG_SAVE
  AC_LANG_CPLUSPLUS
  AC_TRY_COMPILE([#include <math.h>
],[long double x=1; int ret = (int)logl(x); return 0; ],
  ac_cv_cxx_have_logl=yes, ac_cv_cxx_have_logl=no)
  AC_LANG_RESTORE
])
if test "$ac_cv_cxx_have_logl" = yes; then
   AC_DEFINE(HAVE_LOGL,,[define if long double logl works.])
fi
])
